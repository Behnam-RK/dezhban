...
type Policy struct {
	...
	// leak is DNS-query metadata only, and the flag is a documented default-ON
	// trade (2026-07 defaults review: redialability wins for this project's
	// users) a user can turn off.
	//
	...
	// tolerated here only because it is a deliberate, user-visible setting that
	// solves a tunnel-DOWN problem no tunnel-scoped rule can. See the "known,
	// deliberate exception" note in docs/adr/0006 before adding any further
	// port-53 rule.
