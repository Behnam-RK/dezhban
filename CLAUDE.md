...
`switch`, `pause`, `resume`, `vpn`, `setup`, `config`, `completion`, `upgrade`, `version`,
plus a global `-v`/`--verbose`. `validate`, `print-rules`, `doctor`, `monitor`, and
`upgrade check` are read-only (no root, no firewall effects); the rest of the
privileged set requires root/admin, including `upgrade download`/`upgrade apply`
(macOS only — `download`'s staging directory is root-owned so a local user can't
swap the verified `.pkg` before `apply` installs it). `switch`, `pause`, and
`resume` are privileged but usually passwordless: they ask the running daemon
...
command file when no daemon answers. Full reference: [docs/usage.md](docs/usage.md);
the upgrade design in full: [docs/upgrade.md](docs/upgrade.md).
